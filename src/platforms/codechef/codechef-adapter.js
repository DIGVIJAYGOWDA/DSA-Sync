...
export class CodeChefAdapter extends BaseAdapter {
  ...
  extractTitle() {
    const selectors = ['h1[class*="problem"]', '.problem-code', 'h1', 'h2'];
    for (const selector of selectors) {
      const el = document.querySelector(selector);
      if (el && el.textContent.trim()) {
        const text = el.textContent.trim();
        if (!['CodeChef', 'Practice', 'Compete', 'Discuss'].includes(text)) return text;
      }
    }
    if (document.title) return document.title.split('|')[0].split('-')[0].trim() || 'CodeChef Problem';
